...
class BBBConv2d(_ConvNd):
    ...
    def convprobforward(self, input):
        ...
        conv_qw_mean = F.conv2d(input=input, weight=self.qw_mean, stride=self.stride, padding=self.padding,
                                dilation=self.dilation, groups=self.groups,bias=self.qb_mean)
        biast = torch.exp(self.log_alpha)*self.qb_mean.pow(2)
        ss = biast.shape[-1]
        biast = torch.reshape(biast, [ss])
        conv_qw_si = torch.sqrt(1e-8 + F.conv2d(input=input.pow(2), weight=torch.exp(self.log_alpha)*self.qw_mean.pow(2),
                                                stride=self.stride, padding=self.padding, dilation=self.dilation, groups=self.groups,bias=biast))
        ##########################
        if cuda:
            conv_qw_mean.cuda()
            conv_qw_si.cuda()
...
class BBBLinearFactorial(nn.Module):
    ...
    def fcprobforward(self, input):
        ...
        """
    ###20230606添加bias的改动
        fc_qw_mean = F.linear(input=input, weight=self.qw_mean,bias=self.qb_mean)
        biast = torch.exp(self.log_alpha)*self.qb_mean.pow(2)
        ss = biast.shape[-1]
        biast = torch.reshape(biast, [ss])
        fc_qw_si = torch.sqrt(1e-8 + F.linear(input=input.pow(2), weight=torch.exp(self.log_alpha)*self.qw_mean.pow(2),bias=biast))
    ##########################
        if cuda:
            fc_qw_mean.cuda()
            fc_qw_si.cuda()
